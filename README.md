...
![IndoorLoRa System Architecture](images/system_architecture.png)
...
| **HMM-Viterbi** | Online Viterbi tracking with Gaussian motion-continuity transition model | **~0.86 m** |
...
### Trajectory — raw N-Lat vs HMM smoothed track

![Estimated Trajectory](images/trajectory.png)

### Per-anchor RSSI signal over time

![Raw RSSI Logs](images/rssi_logs.png)
...
![OSM Map Mode](images/dashboard_osm.png)
...
|--------------------|--------------------|
| ![New Location](images/new_location.png) | ![Location Switcher](images/location_switcher.png) |
...
InDoorLora/
│
├── positioning_server.py       # Core: ZMQ → Kalman → Algorithms → SQLite → WebSocket
...
│   ├── fp.py                   # K-NN Fingerprinting
│   └── hmm.py                  # HMM-Viterbi
│
├── emertters/                  # Arduino firmware (Arduino IDE)
...
├── images/                     # All figures and screenshots
│
├── tests/
│   └── test_projection.py
│
...
- GNU Radio with a LoRa demodulation block + USRP B200mini drivers (UHD)
- Arduino IDE with the [RadioHead](http://www.airspayce.com/mikem/arduino/RadioHead/) library (for anchor firmware)

### Run with live hardware (USRP B200mini)

```bash
# Terminal 1 — start the USRP bridge (subscribes to GNU Radio, publishes to ZMQ)
...
Installs Miniconda, creates the `indoorlora` conda environment, installs GNU Radio + UHD + gr-lora + all Python dependencies, and downloads USRP FPGA images.
...
4. **Export radio map** — click **Export Map**. Fingerprinting and HMM become active once ≥ 3 reference points are calibrated.
5. **Switch locations** — use the Location dropdown; all data is persisted per-location in SQLite.
